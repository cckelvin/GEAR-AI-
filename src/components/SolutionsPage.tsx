...
import { Target, Users, Landmark, Rocket, Sparkles, GraduationCap } from 'lucide-react';
    ...
    <div className="flex-1 flex flex-col p-8 overflow-y-auto custom-scrollbar bg-[#0A0A0A] text-white relative">
      ...
      <div className="max-w-4xl mx-auto w-full space-y-12 py-8 relative z-10">
        ...
        <div className="text-center space-y-3 pb-6 border-b border-[#1F1F1F]">
          <span className="text-[10px] font-black uppercase text-indigo-400 tracking-[0.2em]">Target Audiences</span>
          <h1 className="text-3xl md:text-4xl font-black tracking-tight">Tailored Solutions</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {personas.map((p, idx) => (
            <div 
              key={idx}
              className="p-6 bg-[#0F0F0F] border border-[#1F1F1F] rounded-2xl space-y-3 hover:border-indigo-500/40 transition-colors"
            >
              <div className="flex items-center gap-3">
                <div className="p-2.5 bg-indigo-500/5 border border-indigo-500/15 rounded-xl text-indigo-400">
                  {p.icon}
                </div>
                <div>
                  <h3 className="font-bold text-sm text-white">{p.audience}</h3>
                  <span className="text-[9px] font-bold uppercase text-indigo-400 tracking-wider block mt-0.5">{p.goal}</span>
                </div>
              </div>
              <p className="text-xs text-gray-400 leading-relaxed pt-1">{p.desc}</p>
            </div>
          ))}
        </div>
      </div>
